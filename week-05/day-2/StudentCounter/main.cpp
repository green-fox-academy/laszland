...
std::vector<std::string> getChildrenWithMoreThanFourCandies(std::vector<std::pair<std::string, std::map<std::string, int>>>& students)
{
    std::vector<std::string> returnVector;

    for (int i = 0; i < students.size(); ++i) {
        for (std::pair<std::string, int> p: students[i].second) {
            if (p.first == "candies" && p.second > 4) {
...
int sumOfAgeWithLessThanFiveCandies(std::vector<std::pair<std::string, std::map<std::string, int>>>& students)
{
    int sumOfAge = 0;
    for (int i = 0; i < students.size(); ++i) {
        for (std::pair<std::string, int> p: students[i].second){
        if (p.first == "candies" && p.second < 4) {
            sumOfAge += students[i].second["age"];
        }
        }
    }
    return sumOfAge;
